      ...
      <TabItem Header="Voice and face">
        <ScrollViewer VerticalScrollBarVisibility="Auto" Padding="14">
          <StackPanel>
            ...
            <TextBlock Text="Dashboard URL" Style="{StaticResource Head}" />
            <TextBox x:Name="DashboardUrlBox" />
            <TextBlock Style="{StaticResource Hint}"
                       Text="A web page on a button over her — a Home Assistant dashboard, say. Empty hides the button. It is framed, so the page must allow that: Home Assistant sends X-Frame-Options and shows blank until that is switched off at its end. Use the same scheme her face is served over." />
