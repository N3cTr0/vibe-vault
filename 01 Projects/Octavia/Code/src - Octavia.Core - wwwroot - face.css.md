...
project: Octavia
tags: [octavia, code]
source-path: src\Octavia.Core\wwwroot\face.css
---

# src\Octavia.Core\wwwroot\face.css
...
/* ── notice ─────────────────────────────────────────────── */
/* The connection bar. Across the top rather than beside the state pill: losing her is not
   a state she is in, it is this face no longer knowing what state she is in — and the pill
   would go on cheerfully reporting the last thing it was told. */
#link{
  position:fixed;top:0;left:0;right:0;z-index:9;
  font-family:var(--mono);font-size:var(--t-meta);letter-spacing:.1em;text-transform:uppercase;
  text-align:center;padding:8px 12px;
  background:rgba(36,34,25,.94);color:#F6F3EA;
  transform:translateY(-100%);transition:transform .25s;
}
#link.on{transform:translateY(0)}
#link .d{
  display:inline-block;width:6px;height:6px;border-radius:50%;
  background:var(--warn);margin-right:9px;vertical-align:middle;
}
