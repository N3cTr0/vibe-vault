...
The contract between the host (`Octavia.Server.exe`, the being) and a face (a renderer).
JSON messages, each with a `type`. Anything that speaks this protocol is a legal face: the
Windows client, a browser on another machine, the Android client, or — later — an Unreal
application rendering a photoreal character.
...
> **Since v0.26.0 the host is a separate process and *every* face is a socket face**,
> the Windows client included. The client is a window around this page and nothing else: it
> holds no session, no brain and no voice, and reaches her exactly the way a phone does.
> Nothing on the wire changed to make that true, which was the whole claim of Stage 3.

## Transports

| Transport | Used by | Notes |
|---|---|---|
| WebSocket | Every face | `ws://<host>:<FacePort>/?token=<token>` — or `?key=<remote key>`, which is what a client uses because the token is minted fresh on every start |
| HTTP `GET` | Every face | The same port serves the page itself. See *Serving the face* below |

Several faces can be attached at once — useful when developing a new renderer beside the
old one, and the intended arrangement with a tablet, a phone and the desktop all attached.
Since v0.24.0 a face belongs to a **room**, and most messages go to a room rather than to
everyone; see *Rooms* below.

> **There used to be a third transport**: WebView2 `postMessage`, for the page hosted inside
> the host's own process, used automatically when the socket could not be bound. v0.26.0
> removed it along with the process that answered it. A page that fell back to it now would
> report success into a void — and it suppressed the *"lost the connection"* notice for
> exactly the face that most needs one.
...
Every renderer needs the files over HTTP, so **the same port answers plain GETs** that are
...
as `127.0.0.1` — that is the host's loopback, not the client's. `?port=` overrides this,
for a page served from somewhere other than the socket it should talk to.
...
> **Loopback is the exception, and it is why the Windows client works.** `http://127.0.0.1`
> *is* a secure context by the same rule that makes `localhost` one, so a client on the
> machine the server runs on has a camera and the LAN does not — with no `https` anywhere.

**A face must re-announce `ready` on every reconnection**, not only the first. A socket that
drops and comes back is a *new* face to the host: new `FaceId`, no room, no senses, nothing
remembered. A renderer that announced once would find itself silently back in the host room
with no camera, working and wrong.
...
Windows client and every renderer written before this keep behaving exactly as they did. A
...
| Type | Fields | Meaning |
|---|---|---|
| `hello` | `protocol`, `room`, `controls`, `hasKey`, `model`, `profile`, `ears`, `voice`, `listening`, `avatar?`, `avatarFile`, `avatars[]`, `roomHour`, `music`, `musicAvailable`, `camera`, `cameraDevice`, `stats`, `microphones[]`, `microphone`, `outputs[]`, `output`, `whisperCompute`, `toolServers[]`, `audioAvailable`, `audioRate`, `audioBits`, `audioChannels`, `micAccepted`, `dev`, `state`, `emotion`, `emotionWeight` | Capabilities and current settings, **for this face**. Sent on connect and whenever they change. `room` is the room this face was put in, echoed back so a typo in `?room=` is visible rather than mysterious; `controls` is `host` or `room` — see *Rooms*. `state`, `emotion`, `camera` and `cameraDevice` are that room's, not the session's. `avatar` is a URL for a VRM character (absent when there is none); `avatars[]` is what the host has to offer and `avatarFile` which is chosen. `microphones[]` and `outputs[]` are `{value, label}` too, where an empty `value` means "follow the Windows default" and is always offered first; the label marks which one that currently is. `toolServers[]` is `{name, ready}` — reported even before she can call them, because "is the integration actually connected" should not need a log to answer. The four `audio*` fields describe her voice as a stream: `audioAvailable` is false for a voice that cannot be teed, and a face is told so rather than left waiting for frames that were never coming — no voice she has can fail this now, and the field stays because that is a fact about the engine rather than a promise about every future one. `audioRate` comes from the live voice model, so it **changes when the voice does** — re-read it on every `hello` rather than caching it once. `micAccepted` says whether the host **will** take audio *from* a face — not whether it already has. It reports what the recogniser is or could be, so a handset is not told its microphone button is useless on a fresh session, when in fact holding that button is what opens her ears. |
...
right. Audio is a **physical output**. If it were opt-out, every browser face on the
server's own machine would start playing her voice on top of the speakers she is already
using — not a bandwidth problem but her talking over herself in the same room. A face that
draws her mouth has not thereby claimed the right to make noise.

> **v0.26.0 made this matter to the desktop as well.** The host used to refuse to stream to
> the page it hosted, because that page shared this machine's speakers. There is no such
> page now: the Windows client is a socket face, and if it wants to hear her it asks like
> everyone else.
>
> On one machine nothing changes, and it is worth being precise about why: the client is in
> the **host room**, so `Aloud` is true and the *server* plays her through the sound card it
> is sitting at. You hear her because she is speaking in the room you are in, not because
> anything was streamed to the window.
>
> **A client on another machine is where that stops being true**, and it is the sharp edge
> of Stage 15 item 3: the host room would then contain a face that is not in it, the server
> would talk aloud to an empty desk, and a Windows voice could not follow — SAPI cannot be
> streamed, so `audioAvailable` is false and she says so in a `notice`. Until item 3 is
> built, **run the client on the machine the server is on, or use the neural voice.**
...
> `look` can arrive at any moment, including while a watcher is running. Her page
...
| ~~`setVoice`~~ | — | **Gone, v0.40.0.** She has one voice, chosen by ear out of twenty-two in Stage 16. A menu over a list of one is a control that cannot do anything. |
| ~~`setVoiceEngine`~~ | — | **Gone, v0.40.0.** There has been one engine since v0.37.0 and now there is one voice inside it. Both are still *answered* — an old face that sends either is told so out loud, rather than having it land in `default` and be reported as not understood. |
...
- **A client uses the remote key rather than the token**, even on loopback, and
  `Authorised` has always allowed that. The token is minted fresh at every start, so a
  client holding one would need re-pairing every time the server restarted — which is the
  same reason a phone uses the key, arriving at the same answer from the other direction.

The key itself never crosses the protocol outward: `setKey` carries it *in*, and `hello`
reports only a boolean.

`saveDiagnostics` asks for a bundle but cannot say **where**: the host writes it into
`data/diagnostics/` under a timestamped name and answers with the path. A face that could
name the destination would be a face that could write a file containing the log anywhere on
the machine.

> **It used to raise a file dialog, and that could not survive v0.26.0.** A dialog needs a
> dispatcher and somebody looking at it, and the host may now be running where there is
> neither — so the one control that exists for when she is broken would have been broken by
> moving her. The path travels back over the socket instead, which also means the person who
> asked is told even when they are in another room. `diagnosticsSaved` carries it.
