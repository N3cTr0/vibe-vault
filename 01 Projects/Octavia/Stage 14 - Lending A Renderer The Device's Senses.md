...
> **Confirmed working from the handset, with one fault left open** *(09/01/2026)*. `micAccepted: True` on a fresh session and the button opened her ears from the phone. But the first press of a cold session loses its opening words — nothing acknowledges the floor, so the ~3 seconds streamed while Whisper loads are dropped, and the result is a plausible truncated sentence rather than an obvious silence. **[[Roadmap]] item 11**, recorded and not yet built; the client cannot fix it, and the likely shape is host-side buffering with no protocol change.
