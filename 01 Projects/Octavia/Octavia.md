...
- **Current version:** 0.47.0 (pre-release `0.x` scheme — see [[Conventions & Security Model]])
...
- **Distribution:** **three** exes in `dist\` — `Octavia.Server.exe` (her, and her tray), `Octavia.exe` (the client) and `octavia-kokoro.exe` (her voice). See [[Build & Release]] — **leave the last one out and she runs but cannot speak**
...
>
> **And then 0.41.0 → 0.46.0, the same day.** The shape of it:
>
> - **She can change something** (0.41.0). `power_cycle_port` is the first tool in the project
>   that is not a read, and it is a `Confirm`: she asks plainly and stops. See [[Hands]].
> - **She can speak first** (0.42.0). `Watchman` walks a route on a clock and `SayUnprompted`
>   is a turn she begins — the first thing she has ever said that nobody asked for. See
>   [[Her Rounds]].
> - **A tool server can be given a secret** (0.43.0) without it being written in
>   `config.json`. DPAPI-sealed, filled into the child's environment at spawn. See
>   [[Conventions & Security Model]].
> - **She watches the network for a week before saying anything about it** (0.44.0), and
>   learns what normal is *wherever she is installed* — nothing about this house is in her.
> - **A log file per day**, purged after a fortnight (0.45.0). And **which IPS rule matched is
>   not obtainable at all**, settled by probing — see [[UniFi API]].
> - **Her server has a tray icon and a settings window** (0.46.0). **The client stopped
>   starting and stopping her** — a client sets what it sends out, not what runs elsewhere.
> - **And it is one executable again** (0.47.0). The tray was split into its own app on the
>   reasoning that a service has no desktop; that is an argument for a second *mode*, not a
>   second *binary*. `Octavia.Server.exe` with no arguments is now her tray. See
>   [[Her Controls]].
...
> **A second, and it looks exactly like a fault:** she is **silent about the network until
> roughly 09/10/2026** by design, while she learns what normal is. [[Her Rounds]] explains it,
> and Health → Rounds says how far through she is. **Do not "fix" the silence.**
...
- [[The Voice]] — Kokoro `af_heart` and no other, and lip sync read out of the audio
...
- [[Hands]] — the MCP seam, the UniFi tools, and the first one that changes something
- [[Her Rounds]] — **the one thing she does that nobody asked for**, and why the answer is almost always silence
- [[Her Controls]] — her server's tray icon and settings window, in a process of its own
...
- [[UniFi API]] — all three UniFi surfaces, what each can and cannot do, and how that was established. **Read before probing the gateway again**
