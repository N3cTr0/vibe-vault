...
- **Current version:** 0.46.0 (pre-release `0.x` scheme — see [[Conventions & Security Model]])
...
- **Distribution:** **four** exes in `dist\` since v0.46.0 — `Octavia.Server.exe`, `Octavia.exe` (the client), `Octavia.Control.exe` (her server's tray icon and settings) and `octavia-kokoro.exe` (her voice). See [[Build & Release]] — **leave either of the last two out and she runs but cannot speak, or cannot be configured**
...
> - **Her server has a tray icon and a settings window** (0.46.0), in a process of its own
>   because a Windows service has no desktop. **The client stopped starting and stopping her.**
>   See [[Her Controls]].
