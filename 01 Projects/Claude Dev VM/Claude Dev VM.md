...
- 2026-07-18 — **confirmed Claude can launch + drive PartnerTool's WPF UI.** In an interactive (RDP) session WPF renders fine; `tools\ui-drive.ps1` navigates pages by AutomationId and screenshots (verified System Info → Diagnostics → Network). **Claude Code must run elevated** — not only to launch the `requireAdministrator` app, but because Windows **UIPI** blocks a medium-integrity process from injecting input into the elevated window. Relaunch as admin (silent under "Never notify").
