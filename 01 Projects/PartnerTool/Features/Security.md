...
- **Per-collector timings go to the activity log** — `page load: audit N ms, defender N ms, prosentry N ms, bitlocker N ms` — so the next slow machine names its own culprit in its diagnostics bundle.
